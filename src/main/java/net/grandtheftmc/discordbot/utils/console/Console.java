...
import net.grandtheftmc.discordbot.utils.console.commands.listeners.ConsoleHelpCommand;
import net.grandtheftmc.discordbot.utils.console.commands.listeners.ConsoleMemoryCommand;
import net.grandtheftmc.discordbot.utils.console.commands.listeners.ConsoleQueryCommand;
import net.grandtheftmc.discordbot.utils.console.commands.listeners.ConsoleStopCommand;
...
public class Console {
    ...
    public static void loadShutdownHook() {
        Runtime.getRuntime().addShutdownHook(new Thread( () -> {
            // close jda
            if (GTMBot.getJDA() != null) {
                // Shut down JDA
                Logs.log("Shutting down discord bot...");
                GTMBot.getJDA().shutdownNow();
            }
            // Close database pool connection
            Logs.log("Closing connections to databases...");
            for (BaseDatabase.Database database : BaseDatabase.Database.values())
                BaseDatabase.getInstance(database).close();
            GTMBot.getJedisManager().close();
        }));
    }
